...
return function(s)
          ...
          {
            {
              text = shortname,
              widget = wibox.widget.textbox,
              font = "Cascadia Code PL, ExtraBold 12",
              id = "shortname"
            },
            fg = color["bg_red"],
            widget = wibox.container.background,
        ...
        awful.spawn.easy_async_with_shell(
          ...
          function(stdout)
            ...
            if kb_layout_item.keymap == layout then
              kb_layout_item.bg = color["bg_aqua"]
              kb_layout_item:get_children_by_id("background2")[1].fg = color["bg"]
              kb_layout_item:get_children_by_id("background1")[1].fg = color["bg"]
            else
              kb_layout_item.bg = color["Grey800"]
              kb_layout_item:get_children_by_id("background2")[1].fg = color["bg_orange"]
              kb_layout_item:get_children_by_id("background1")[1].fg = color["bg_aqua"]
            end
          end
        )
      end
  ...
  local kb_menu_widget = awful.popup {
    ...
    fg = color["fg"],
    border_width = dpi(4),
    border_color = color["bg_aqua"],
    width = dpi(100),
    max_height = dpi(600),
    visible = false,
    ontop = true,
    placement = function(c) awful.placement.align(c, { position = "top_right", margins = { right = dpi(40), top = dpi(50) } }) end
  }
  ...
  kb_menu_widget:connect_signal(
    ...
    function()
      mousegrabber.run(
        function()
          kblayout_widget.bg = color["bg_aqua"]
          awesome.emit_signal("kblayout::hide:kbmenu")
          mousegrabber.stop()
          return true
        end,
