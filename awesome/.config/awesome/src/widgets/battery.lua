...
return function()
        ...
        {
          {
            {
              id = "icon",
              image = gears.color.recolor_image(icondir .. "battery-unknown.svg", color["bg"]),
  ...
  local battery_warning = function()
    naughty.notification {
      icon = gears.color.recolor_image(icondir .. "battery-alert.svg", color["fg"]),
  ...
  local update_battery = function(status)
    awful.spawn.easy_async_with_shell(
      ...
      function(stdout)
        ...
        battery_widget.container.battery_layout.spacing = dpi(2)
  ...
  -- Hover_signal(battery_widget, color["bg_purple"], color["bg"])
