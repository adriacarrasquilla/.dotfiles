...
return function()
  ...
  local network_notify = function(message, title, app_name, icon)
    naughty.notification {
      ...
      title = title,
      app_name = app_name,
      icon = gears.color.recolor_image(icon, color["fg"]),
      timeout = 3
    }
  end
    ...
    local update_wireless_icon = function(strength)
      awful.spawn.easy_async_with_shell(
        ...
        function(stdout)
          ...
          network_widget.container.network_layout.icon_margin.icon_layout.icon:set_image(gears.color.recolor_image(icondir .. icon .. ".svg", color["bg"]))
        end
      )
    end
