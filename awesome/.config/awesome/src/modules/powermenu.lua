...
local font = "Cascadia Code Bold 20"
...
return function(s)
  ...
  local update_user_name = function()
    ...
    awful.spawn.easy_async_with_shell(
      "./.config/awesome/src/scripts/pfp.sh 'userName' '" .. namestyle .. "'",
      function(stdout)
        if stdout:gsub("\n", "") == "Rick Astley" then
  ...
  local shutdown_button = button("Shutdown", icondir .. "shutdown.svg", color["bg_blue"], shutdown_command)
  local reboot_button = button("Reboot", icondir .. "reboot.svg", color["bg_red"], reboot_command)
  local suspend_button = button("Suspend", icondir .. "suspend.svg", color["bg_yellow"], suspend_command)
  local logout_button = button("Logout", icondir .. "logout.svg", color["bg_green"], logout_command)
  local lock_button = button("Lock", icondir .. "lock.svg", color["bg_orange"], lock_command)
